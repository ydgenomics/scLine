...
# 在 install.packages() 之前加一行
dir.create("~/R-lib", showWarnings = FALSE, recursive = TRUE)
.libPaths("~/R-lib")   # 把个人库置顶
install.packages(db, repos = NULL, type = "sources", lib = "~/R-lib")
...
for(gene_csv in gene_csvs){
    ...
    for(i in unique(markers$cluster)){
        ...
        if (nrow(data) > 0) {
            ...
            write.table(data, file = paste0(prefix, "_", i, "_enrich.txt"), sep = "\t", row.names = FALSE, col.names = TRUE, quote = FALSE)
            ...
            pdf(paste0(prefix, "_", i, "_plot1.pdf"))
